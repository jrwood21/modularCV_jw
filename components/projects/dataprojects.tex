\begin{longtable}{>{\raggedright\arraybackslash}p{5.5cm} >{\raggedright\arraybackslash}p{3.3cm} p{9cm}}	
	...
	\midrule
	\endhead
	Brain stimulation: effect on electroencephalography (EEG) power 
	...
	\addlinespace
	Brain stimulation: effects on perception \& learning over time 
	...
	\addlinespace
	Brain stimulation: effects on motor-evoked potential (MEP) size \& variability over time
	...
	\addlinespace
	Brain stimulation: effects on MEP size \& variability over time - model validation 
	...
	Using these data, evaluate the performance of competing statistical models to compare changes in the mean and within-individual variability of lognormal data between treatment groups over time.\\
	\addlinespace
	Brain stimulation: effect on rate of change of MEP size over time
	& tidyverse, nlme, boot 
	& Estimate treatment effects on the rate that a response variable (MEP peak-to-peak amplitude) increases over time towards a maximal plateau, 
	...
	& Execute interactive learning tasks (associative word learning, sequential finger tapping) that present stimuli to participants and record responses (voice recordings, key presses) to assess cognitive and motor learning. 
